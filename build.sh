...
if [ -z "$GLPI_VERSION" ]; then
  if ! GLPI_VERSION=$(curl --fail -s https://api.github.com/repos/glpi-project/glpi/releases/latest | \
                     grep --perl-regexp --only-matching '(?<=tag_name": ")[a-z0-9.-]+'); then
...
docker build --build-arg "GLPI_VERSION=${GLPI_VERSION}" \
             ...
             --label 'org.label-schema.vendor=Wolvverine thx Pierre GINDRAUD' \
